...
describe('Sudoku controllers', function(){
  ...
  describe('SudokuCtrl', function() {
    ...
    beforeEach(function() {
      scope = {};
      sudokuFactory = {
        newGame : function() { return gameObj; },
        verifyAnswer : function() { return true }
      };
      module('sudoku')
      ctrl = new SudokuCtrl(scope, sudokuFactory);

      //spyOn(sudokuFactory, 'newGame');
      //sudokuFactory.newGame("medium");
    });
    ...
    it('should start the game', function() {
      expect(scope.gameObj).toEqual(gameObj);
    });

    xit('should update the game board if user selects different difficult level', function() {
      expect(scope.changeLevel("medium")).toHaveBeenCalled();
    });
